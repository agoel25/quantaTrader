...
struct MarketEvent : public Event {
    uint32_t symbol_id;
    explicit MarketEvent(uint32_t symbol_id) : symbol_id(symbol_id) {}
};

struct SymbolAdded : public MarketEvent {
    std::string name;
    SymbolAdded(uint32_t symbol_id, std::string name) : MarketEvent(symbol_id), name(std::move(name)) {}
...
struct SymbolDeleted : public MarketEvent {
    std::string name;
    SymbolDeleted(uint32_t symbol_id, std::string name) : MarketEvent(symbol_id), name(std::move(name)) {}
